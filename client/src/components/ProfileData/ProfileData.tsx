...
const ProfileData = ({ setEditProfile, searchedUser }) => {
  const { user, setUsers, users, setUser } = useUsers();

  function handleToggleFollow(e) {
    let updatedSearchedUser;
    ...
    const updatedUsers = users.map((existingUser) => {
      if (existingUser.id === searchedUser.id) {
        return updatedSearchedUser;
      }
  ...
  return (
    <section className={styles.profileData}>
      ...
      {user.id !== searchedUser.id && (
        <button
          className={styles.profileData__follow}
